...
Scenario Outline: Create language and level record with valid details
	   ...
	   Then New record for Language with '<Language>' and '<Level>' are added successfully
...
Scenario Outline: For negative testing add language record with valid details 
Given User is logged into localhost URL for Language successful
When Add new record '<Language>' and '<Level>' to the language list
Then Record '<Language>' and '<Level>' are added successfully
